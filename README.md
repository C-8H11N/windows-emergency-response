...
> 安全边界：工具只允许监听本机回环地址，并使用随机 HttpOnly 会话保护扫描、处置与报告接口。默认只做本地采集与启发式分析，不自动删除文件或修改注册表，不读取或导出 SAM/LSASS/凭据。威胁情报默认关闭，真实 API 查询必须显式开启。
...
| 📦 **案件包导出** | 导出发现、报告、清单与 SHA-256 校验文件 | 统一案件格式 ZIP |
...
| 💖 **爱发电** | [C-8H11N](https://ifdian.net/a/C-8H11N) | 国内用户小额打赏 |
| ❤️ **GitHub Sponsors** | [Sponsor](https://github.com/sponsors/C-8H11N) | GitHub 官方赞助 |
